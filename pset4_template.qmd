...
4. 
    ...
    #reference: https://stackoverflow.com/questions/30740046/calculate-distance-to-nearest-feature-with-geopandas
    ...
    zips_texas_centroids.to_crs(epsg=4269, inplace=True)
    zips_withhospital_centroids.to_crs(epsg=4269, inplace=True)

    ztc_subset = zips_texas_centroids.head(10)


    #calculate distance to nearest hospital zip
    def calculate_distances(subset_df, pos_df):
      distances = []
      for index, zip_geom in subset_df.iterrows():
        pos_distances = pos_df[pos_df.index != index].distance(zip_geom['geometry'])
        min_distance = pos_distances.min()
        distances.append(min_distance)
      return distances

    start_time = time.time()
    ztc_subset["closest_pos"] = calculate_distances(ztc_subset, zips_withhospital_centroids)
    end_time = time.time()
    ...
    It would take about 19348.05 seconds for the full dataset.
